...
nasm bootload.asm
dd if=bootload of=floppya.img bs=512 count=1 conv=notrunc 2> /dev/null
dd if=map.img of=floppya.img bs=512 count=1 seek=256 conv=notrunc 2> /dev/null
dd if=files.img of=floppya.img bs=512 count=1 seek=258 conv=notrunc 2> /dev/null
dd if=sectors.img of=floppya.img bs=512 count=1 seek=259 conv=notrunc 2> /dev/null
...
rm kernel shell echo mkdir ls rm cat *.o loadFile
